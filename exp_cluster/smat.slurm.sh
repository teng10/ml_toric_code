...
python smat.py --config="${exp_file_path}configs/config_smat.py" --config.job_id=${SLURM_ARRAY_JOB_ID} --config.file_id=${SLURM_ARRAY_TASK_ID} --config.data_dir=${FILEPATH}  > "${FILEPATH}logs/computeS_${SLURM_ARRAY_JOB_ID}_log.txt"
