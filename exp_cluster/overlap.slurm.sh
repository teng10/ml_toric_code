...
python overlap.py --config="${exp_file_path}configs/config_overlap.py" --config.job_id=${SLURM_ARRAY_JOB_ID} --config.file_id=${SLURM_ARRAY_TASK_ID} --config.data_dir=${FILEPATH}  > "${FILEPATH}logs/computeO_${SLURM_ARRAY_JOB_ID}_log.txt"
