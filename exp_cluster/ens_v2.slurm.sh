...
FILEPATH="/n/home11/yteng/experiments/optimization/data/10331255/"
mkdir -p ${FILEPATH}logs/
echo "array_task_id=${SLURM_ARRAY_TASK_ID}"
echo "filepath is ${FILEPATH}"
python ens_v2.py --config="${exp_file_path}configs/config_ens_v2.py" --config.job_id=${SLURM_ARRAY_JOB_ID} --config.file_id=${SLURM_ARRAY_TASK_ID} --config.data_dir=$FILEPATH > "${FILEPATH}logs/${SLURM_ARRAY_JOB_ID}_log.txt"
#python ens_v2.py --config="${exp_file_path}configs/config_ens_v2.py" --config.job_id=${SLURM_ARRAY_JOB_ID} --config.file_id=1 --config.data_dir=$FILEPATH > "${FILEPATH}logs/${SLURM_ARRAY_JOB_ID}_log.txt"
